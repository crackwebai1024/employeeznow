...
function* onSaveFilter({ payload }) {
  try {
    ...
    if (res && res.data) {
      ...
      window.location.href = `/search/${res.data.filterID}`;
...
function* onRemoveFilter({ payload }) {
  try {
    ...
    if (res && res.data) {
      let newFilter = payload.searchQuery.filter(
        (filter) => filter._id !== payload.removeQuery.filterID
      );
      const data = {
        filters: newFilter,
      };
...
function* onChargeRequest({ payload }) {
  try {
    ...
    if (payload.purchasenum === "BUY_SELECT") {
      let freeData = {
        id: payload.id,
        employees: payload.employees.splice(
          0,
