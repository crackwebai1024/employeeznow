    ...
    <div>
      ...
      >
        {searchQuery.map((searchQuery, i) => (
          <ListItem
            button
            onClick={(e) => onFilterClick(searchQuery._id)}
            key={i}
          >
            <ListItemText
              primary={searchQuery.name}
              secondary={searchQuery._id === slug ? "Current Filter" : ""}
            />
            <ListItemIcon>
              <EditOutlinedIcon
...
const mapStateToProps = ({
  employer: { employerData, filter, searchLoading, filterResult },
}) => ({
  employerData,
  filter,
  searchLoading,
